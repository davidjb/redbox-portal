...
do
    if [ $var = "install" ]; then
        docker run -it --rm -v $PWD:$PORTAL_DIR $PORTAL_IMAGE /bin/bash -c "cd $PORTAL_DIR; npm install sails-hook-autoreload && npm -g install typings && npm install"
    fi
    if [ $var = "jit" ]; then
      ...
      docker run -it --rm -v $PWD:$PORTAL_DIR $PORTAL_IMAGE /bin/bash -c "cd $PORTAL_DIR; npm install -g @angular/cli@1.7.1; npm install --only=dev; node_modules/.bin/tsc --project tsconfig.json; cd angular; npm install; make build-frontend"
    fi
    if [ $var = "jit-skip-frontend" ]; then
      #linkNodeLib "lodash" "lodash-lib"
      export ENV=development
      docker run -it --rm -v $PWD:$PORTAL_DIR $PORTAL_IMAGE /bin/bash -c "cd $PORTAL_DIR; npm install -g @angular/cli@1.7.1; npm install --only=dev; node_modules/.bin/tsc --project tsconfig.json;"
    fi
    if [ $var == "aot" ]; then
    ...
    if [[ $var == watch=* ]]; then
      ...
      watch="true"
        ...
        docker exec --detach $RBPORTAL_PS /bin/bash -c "cd /opt/redbox-portal/angular; npm install -g @angular/cli@1.7.1; yarn; ng build --project=${ng2App} --watch --verbose > ${ng2App}-build.log" || exit
        let WATCH_COUNT++
    fi
done
